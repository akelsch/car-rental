...
public class CarTableView extends Application {
	...
	private void initScenes() throws IOException {
		...
		// Employee View
		FXMLLoader loaderEmployeeScene = new FXMLLoader(getClass().getResource(I18nStringsUtil.getEmployeeTableViewURL()),
				I18nUtil.getResourceBundleComponents());
		Parent employeeParent = loaderEmployeeScene.load();
		employeeScene = new Scene(employeeParent);
		
		CarTableViewController carsController = (CarTableViewController) loaderCarScene.getController();
		carsController.setEmployeeScene(employeeScene);
	}
